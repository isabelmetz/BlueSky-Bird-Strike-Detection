...
from simevents import ACDataEvent, BirdDataEvent, RouteDataEvent, PanZoomEvent, \
...
class ScreenIO(QObject):
    ...
    def __init__(self, sim, manager):
        ...
        self.fast_timer.timeout.connect(self.send_bird_data)
    ...
    def send_aircraft_data(self):
        if self.manager.isActive():
            ...
            data.bird_strike = self.sim.cdbirds.counter_strikes
            data.hit_ac      = self.sim.traf.hit_ac

            self.manager.sendEvent(data)

    @pyqtSlot()
    def send_bird_data(self):
        if self.manager.isActive():
            data            = BirdDataEvent()
            data.id         = self.sim.birds.id
            data.lat        = self.sim.birds.lat
            data.lon        = self.sim.birds.lon
            data.alt        = self.sim.birds.alt
            data.tas        = self.sim.birds.tas
            data.trk        = self.sim.birds.trk
