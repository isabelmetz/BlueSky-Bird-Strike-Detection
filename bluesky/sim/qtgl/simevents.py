...
SetNodeIdType, SetActiveNodeType, AddNodeType, SimStateEventType, BatchEventType, PanZoomEventType, ACDataEventType, \
    BirdDataEventType, SimInfoEventType, StackTextEventType, ShowDialogEventType, \
...
class ACDataEvent(QEvent):
    lat = lon = alt = tas = trk = iconf = hit_ac = confcpalat = confcpalon = id = []
    nconf_tot = nlos_tot  = nconf_exp = nlos_exp  = nconf_cur = nlos_cur = bird_strike = 0

    def __init__(self):
        super(ACDataEvent, self).__init__(ACDataEventType)

# iconf, confcpalat, confcpalon
class BirdDataEvent(QEvent):
    lat = lon = alt = tas = trk =  id = []

    def __init__(self):
        super(BirdDataEvent, self).__init__(BirdDataEventType)
