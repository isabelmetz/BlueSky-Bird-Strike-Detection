...
class Route():
    ...
    def addwpt(self,traf,iac,name,wptype,lat,lon,alt=-999.,spd=-999.,afterwp=""):
        ...
        # DESTINATION: Wptype is destination?
            ...
            # required for destination plus keep runway heading
            wplat = lat
            wplon = lon
            wpok  = True
    ...
    def getnextwp(self, traf, index):
        """Go to next waypoint and return data"""


        # the waypoint reached is not yet the last waypoint
        if self.iactwp+1<self.nwp:
            self.iactwp = self.iactwp + 1

            lnavon = True
        # this is the last waypoint
            # make sure the aircraft is deleted
        else:
            lnavon = False


            delta_time = abs(self.traf.gs[index]/self.traf.gr_dec[index])    


            # delete aircraft
            stack.stack("DELAY " + str(delta_time) + " DEL " + str(traf.id[index]))

        
            # and the aircraft just needs a fixed heading to remain on the runway
            # syntax: HDG acid,hdg (deg,True)
            
            if " " in self.wpname[self.iactwp]:
                ...
                stack.stack("HDG " + str(traf.id[index]) + " " + str(wphdg))
                 
                 
                # delay deletion of aircraft by the time required to reduce speed to 0
                #delta_time = abs(self.traf.spd[index]/self.traf.gr_dec[index])    
                #print "deltatime", delta_time, "spd", self.traf.spd[index], "acc", self.traf.gr_dec[index]
    ...
    def findact(self,traf,i):
        ...
        if not self.EH550 and self.wpname[0] == 'EH550':
            iwpnear = 0
            self.EH550 = True
        else:
