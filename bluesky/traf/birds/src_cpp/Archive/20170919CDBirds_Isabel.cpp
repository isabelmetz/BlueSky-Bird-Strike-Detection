            ...
            for (int j = 0; j < size_birds; ++j) {
                ...
                if(*collision_height.ptr > fabs(*alt_ac.ptr - *alt_birds.ptr))
                {
                    ...
                    if(dist < 50.0)
                    {
                        ...
                        {
							//std::cout << "COLLISION BEFORE PACMAN " << dist << " sum of collrads is " <<  collision_radius_ac + collision_radius_birds<<std::endl;
                            // check for pacman
							...
							//std::cout << "INPUT WAS" << rlat_ac <<" " << rlon_ac<<" "  <<rlat_bird<<" " <<rlon_bird<<std::endl;
            				if( (delta_hdg > pacman_low) && (delta_hdg < pacman_high))
            				{
								//std::cout << "COLLISION APPROVED" <<std::endl;
