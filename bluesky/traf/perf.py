...
class CoeffBS:
    ...
    def coeff(self):
        ...
        # dimensions collision detection bird strike
        ...
        for file in files:
            ...
            self.sweep.append(acdoc.find('dimensions/sweep').text) 
...
class Perf():
    ...
    def reset(self):
        ...
        self.mass         = np.array ([])
        self.Sref         = np.array ([])               
        self.traf.sweep        = np.array([]) # needs to be accessed via the traffic class
        ...
        self.traf.gr_acc       = np.array([]) # ground acceleration
        self.traf.gr_dec       = np.array([]) # ground deceleration
    ...
    def create(self, actype):
        ...
        self.traf.sweep        = np.append(self.traf.sweep, float(coeffBS.sweep[self.coeffidx]))
        ...
        self.traf.gr_acc       = np.append(self.traf.gr_acc,coeffBS.gr_acc[self.coeffidx]) # ground acceleration
        self.traf.gr_dec       = np.append(self.traf.gr_dec, coeffBS.gr_dec[self.coeffidx]) # ground acceleration
        ...
        # geometry collision detection bird strike
        self.traf.collision_height = np.append(self.traf.collision_height, coeffBS.collision_height[self.coeffidx])
        self.traf.collision_radius = np.append(self.traf.collision_radius, coeffBS.collision_radius[self.coeffidx])
    ...
    def delete(self, idx):
        ...
        self.traf.sweep   = np.delete(self.traf.sweep, idx)   # sweep
        ...
        self.traf.gr_acc       = np.delete(self.traf.gr_acc, idx)       # ground acceleration
        self.traf.gr_dec       = np.delete(self.traf.gr_dec, idx)       # ground deceleration
    ...
    def perf(self):
       ...
       # self.traf.vs = swaltsel * vspd  
        ...
        self.traf.aspd = np.where((self.traf.alt <0.5)*(self.post_flight)*self.pf_flag, vtas2cas(0.0,0.0), self.traf.aspd)
    ...
    def acceleration(self, simdt):
        ...
        ax = ((self.phase==2) + (self.phase==3) + (self.phase==4) + (self.phase==5) ) \
            *np.minimum(abs(self.traf.delspd / max(1e-8,simdt)), self.traf.ax) + \
            ...
            / max(1e-8,simdt)), self.traf.gr_dec)
        return ax
