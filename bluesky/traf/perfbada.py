...
class Coefficients:
    ...
    def coeff(self):
        ...
        # collision detection bird strikes
        ...
        for f in self.files:
            ...
            # geometry for collision detection bird strikes depending on 
            # wake category. Heavy: A380; Medium: B757-3, Light: DH8D
                '''if "H" in OPFin[680:693]:
                    self.collision_radius.append(79.75/2)
                    self.collision_height.append(1.99/2)
                    print OPFname, "is category H"
                    #print "collision height heavy"
                    
                elif "M" in OPFin[680:693]:
                    self.collision_radius.append(38./2)
                    self.collision_height.append(1.01/2)   
                    print OPFname, "is category M"
                    #print "collision height medium"
                    
                elif "L" in OPFin[680:693]:
                    self.collision_radius.append(28.4/2)
                    self.collision_height.append(1.35/2)  
                    print OPFname, "is category L"
                    #print "collision height low"
                    
                elif "J" in OPFin[680:693]:
                    self.collision_radius.append(79.75/2)
                    self.collision_height.append(1.99/2)
                    print OPFname, "is category A380"
                    
                # if uncertain take medium
                else:
                    self.collision_radius.append(38./2)
                    self.collision_height.append(1.35/2)  
                    print OPFname, "is category NONE"
                   # print "collision height else"'''
                ...
                self.vmic.append(OPFout[18])
                self.vmcr.append(OPFout[14])
                self.vmap.append(OPFout[26])
                self.vmld.append(OPFout[30])
...
class PerfBADA():
    ...
    def reset(self):
        ...
        self.traf.sweep = np.array([]) # needs to be in traf
        ...
        self.traf.gr_acc      = np.array([]) # ground acceleration [m/s^2]
        self.traf.gr_dec      = np.array([]) # ground deceleration [m/s^2]

        # collision detection bird strikes
        self.traf.collision_radius = np.array([])
        self.traf.collision_height = np.array([])
    ...
    def create(self, actype):
        ...
        self.traf.sweep   = np.append(self.traf.sweep, 25.) # no reference value given in BADA
...
#######################################        
        ...
        self.traf.gr_acc      = np.append(self.traf.gr_acc, 2.0) # value from BADA.gpf file
        self.traf.gr_dec      = np.append(self.traf.gr_dec, 2.0) # value from BADA.gpf file
        # for now, BADA aircraft have the same acceleration as deceleration 
        
        # collision detection bird strikes
        self.traf.collision_radius = np.append(self.traf.collision_radius, coeff.collision_radius[self.coeffidx])
        self.traf.collision_height = np.append(self.traf.collision_height, coeff.collision_height[self.coeffidx])
    ...
    def delete(self, idx):
        ...
        self.traf.sweep   = np.delete(self.traf.sweep, idx)
        ...
        self.traf.gr_acc      = np.delete(self.traf.gr_acc, idx)
        self.traf.gr_dec      = np.delete(self.traf.gr_dec, idx)        
        # collision detection bird strike
        self.traf.collision_radius = np.delete(self.traf.collision_radius, idx)
        self.traf.collision_height = np.delete(self.traf.collision_height, idx)
    ...
    def acceleration(self, simdt):
        ...
        ax = ((self.phase==2) + (self.phase==3) + (self.phase==4) + (self.phase==5) ) \
            *np.minimum(abs(self.traf.delspd / max(1e-8,simdt)), self.traf.ax) + \
            ...
            / max(1e-8,simdt)), self.traf.gr_acc)
