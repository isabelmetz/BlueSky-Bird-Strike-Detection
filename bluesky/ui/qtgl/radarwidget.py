...
# Local imports
from ...settings import text_size, apt_size, wpt_size, ac_size, bird_size, font_family, font_weight, text_texture_size
...
class RadarWidget(QGLWidget):
    ...
    def create_objects(self):
        ...
        # and the same for the birds
        self.birdhdgbuf      = create_empty_buffer(MAX_NBIRDS * 4, usage=gl.GL_STREAM_DRAW)
        self.birdlatbuf      = create_empty_buffer(MAX_NBIRDS * 4, usage=gl.GL_STREAM_DRAW)
        self.birdlonbuf      = create_empty_buffer(MAX_NBIRDS * 4, usage=gl.GL_STREAM_DRAW)
        self.birdaltbuf      = create_empty_buffer(MAX_NBIRDS * 4, usage=gl.GL_STREAM_DRAW)
        self.birdtasbuf      = create_empty_buffer(MAX_NBIRDS * 4, usage=gl.GL_STREAM_DRAW)
        self.birdcolorbuf    = create_empty_buffer(MAX_NBIRDS * 4, usage=gl.GL_STREAM_DRAW)
        ...
        # ------- bird symbol -----------------------------
        self.bird_symbol = RenderObject(gl.GL_TRIANGLE_FAN, vertex_count=4)
        birdvertices = np.array([(0.0, 0.5 * bird_size), (-0.5 * bird_size, -0.5 * bird_size), (0.0, -0.25 * bird_size), (0.5 * bird_size, -0.5 * bird_size)], dtype=np.float32)
        self.bird_symbol.bind_attrib(ATTRIB_VERTEX, 2, birdvertices)
        self.bird_symbol.bind_attrib(ATTRIB_LAT, 1, self.birdlatbuf, instance_divisor=1)
        self.bird_symbol.bind_attrib(ATTRIB_LON, 1, self.birdlonbuf, instance_divisor=1)
        self.bird_symbol.bind_attrib(ATTRIB_ORIENTATION, 1, self.birdhdgbuf, instance_divisor=1)
        self.bird_symbol.bind_attrib(ATTRIB_COLOR, 3, self.birdcolorbuf, datatype=gl.GL_UNSIGNED_BYTE, normalize=True, instance_divisor=1)
    ...
    def paintGL(self):
        ...
        # Draw bird symbols
        if self.nbirds > 0 and self.show_traf:
    ...
    def update_aircraft_data(self, data):
        ...
        if self.naircraft == 0:
            ...
            for i in range(self.naircraft):
                ...
                if data.hit_ac[i] ==i:
                    #print"in radarwidget, here we have", data.hit_ac
                    color[i,:] = red
...
#
###############################################################################
# what is data, which format --> it is a class
# called in gui
    def update_bird_data(self, data):
        ...
        if self.nbirds == 0:
            ...
            # Update data in GPU buffers
            update_buffer(self.birdlatbuf, np.array(data.lat, dtype=np.float32))
            update_buffer(self.birdlonbuf, np.array(data.lon, dtype=np.float32))
            update_buffer(self.birdhdgbuf, np.array(data.trk, dtype=np.float32))
            update_buffer(self.birdaltbuf, np.array(data.alt, dtype=np.float32))
            update_buffer(self.birdtasbuf, np.array(data.tas, dtype=np.float32))
...
####################no label required for the  birdies
           ...
           #     if len(confindices) > 0:
            ...
            # delete if no label required?
#            update_buffer(self.birdlblbuf, np.array(rawlabel, dtype=np.string_))
