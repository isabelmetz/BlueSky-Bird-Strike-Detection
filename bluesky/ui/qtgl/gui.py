...
from ...sim.qtgl import PanZoomEvent, ACDataEvent, BirdDataEvent, StackTextEvent, \
                     PanZoomEventType, ACDataEventType, BirdDataEventType, SimInfoEventType,  \
...
# import BirdDataEventType, BirdDataEvent
                ...
                if self.nd.bird_id in event.id:
                    ...
                    # TAS required???
                    tas = event.tas[idx]
                    self.nd.update_birds_data(idx, lat, lon, tas, trk, len(event.lat))
                ...
                if manager.sender()[0] == manager.actnode():
                    ...
                    self.win.siminfoLabel.setText(u'<b>t:</b> %s, <b>\u0394t:</b> %.2f, <b>Speed:</b> %.1fx, <b>UTC:</b> %s, <b>Mode:</b> %s, <b>Aircraft:</b> %d, <b>Conflicts:</b> %d/%d, <b>LoS:</b> %d/%d, <b>Bird Strikes:</b> %d'
                        % (simt, event.simdt, event.sys_freq, simtclock, self.modes[event.mode], event.n_ac, self.acdata.nconf_cur, self.acdata.nconf_tot, self.acdata.nlos_cur, self.acdata.nlos_tot, self.acdata.bird_strike ))                    
                return True
